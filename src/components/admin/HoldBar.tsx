...
export function HoldBar({ hold, isOwner, onChange, toast }: Props) {
  ...
  // The wrapper carries the state so the page header can keep the button beside the pills and
  // give the notice or the dialog a full-width row of its own.
  if (hold.on) {
    return (
      <div className="hold-bar on">
  ...
  return (
    <div className={`hold-bar${open ? " open" : ""}`}>
      {open ? (
        <div className="notice info hold" role="dialog" aria-label="Hold prices">
          ...
          <div className="btn-row">
            ...
            <button className="btn outline" type="button" disabled={busy} onClick={() => setOpen(false)}>
              Cancel
            </button>
          </div>
        </div>
      ) : (
        <button className="btn small outline hold-btn" type="button" onClick={() => setOpen(true)} title="Hide every price from customers until new rates are published">
          Hold prices
        </button>
      )}
    </div>
